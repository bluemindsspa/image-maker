...
class Exportacion(models.Model):
    ...
    exportacion = fields.Many2one(
        ...
        comodel_name="account.move.exportacion",
        readonly=True,
    ...
    @api.model
    def crear_exportacion(self, move_id):
        exportacion = self.env['account.move.exportacion'].create({
            'name': 'Exportación',
            'currency_id': self.currency_id.id,
            #'move_id': move_id,
        })
        self.exportacion = exportacion.id
        self.write({'exportacion': exportacion.id})
    ...
    def currency_target(self):
        if self._es_exportacion():
            clp = self.env.ref('base.CLP').with_context(date=self.invoice_date)
    ...
    def _totales_otra_moneda(self, currency_id, MntExe, MntNeto, IVA, TasaIVA,
        ...
        if MntExe:
            if currency_id:
                MntExe = base._convert(MntExe, self.company_id.currency_id, self.company_id, self.invoice_date)
            Totales['MntExeOtrMnda'] = MntExe
        if currency_target:
            MntTotal = base._convert(MntTotal, self.company_id.currency_id, self.company_id, self.invoice_date)
    ...
    def _validaciones_uso_dte(self):
        ...
        if self._es_exportacion():
            ...
            if not self.invoice_payment_term_id.modalidad_venta and not self._es_nc_exportacion():
    ...
    def get_monto_clausula(self):
        mnt_clau = self.invoice_payment_term_id.with_context(currency_id=self.currency_id.id).compute(
            self.amount_total, date_ref=self.invoice_date)[0]
        return round(mnt_clau[1], 2)
